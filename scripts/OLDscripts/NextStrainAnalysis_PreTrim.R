...
# Trim alignment
alignment <- read.dna("intermediatedata/BRaligned.afa", "fasta")
#checkAlignment(test, plot = FALSE)
trimmed_alignment <- trimEnds(alignment, min.n.seq = 100) # Variable to play with (see ?trimEnds documentation). This affects # of objs in uniq_samples by trimming
...
aligned_df <- data.frame(ID = gsub("Sample[ _]", "S", aligned_seqs@ranges@NAMES), 
...
IonSeq<-grep("^S[0-9]+$",aligned_df$ID)
NanoSeq<-grep("^S[0-9].*nanopolish.*",aligned_df$ID)
aligned_df$ID[IonSeq]<-gsub("(S[0-9]+).*","\\1i",aligned_df$ID[IonSeq])
aligned_df$ID[NanoSeq]<-gsub("(S[0-9]+).*","\\1n",aligned_df$ID[NanoSeq])
...
#-------------------------------------Sorting into reference sequences-----------------------------
